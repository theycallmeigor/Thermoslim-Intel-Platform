...
export function OrderTable({ orders }: { orders: OrderData[] }) {
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <td className="px-6 py-3.5">
                <span className="text-blue-400 font-medium font-mono text-xs">{order.sourceOrderId}</span>
              </td>
              <td className="px-6 py-3.5 text-gray-300 text-xs">
                {order.customer.fullName || order.customer.email}
              </td>
              <td className="px-6 py-3.5">
                <Badge label={order.sourceLabel} colorClass={order.sourceColor} />
              </td>
              <td className="px-6 py-3.5">
                {order.orderType !== 'one-time' && (
                  <Badge
                    label={order.orderType === 'rebill' ? 'Rebill' : 'Subscription'}
                    colorClass={order.orderType === 'rebill' ? 'bg-orange-500/10 text-orange-400' : 'bg-purple-500/10 text-purple-400'}
                  />
                )}
              </td>
              <td className="px-6 py-3.5">
                <Badge label={order.statusLabel} colorClass={order.statusColor} />
              </td>
        ...
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setSelectedOrder(null)}>
          ...
          >
            ...
            <div className="sticky top-0 bg-gray-950 border-b border-gray-800 px-6 py-4 flex items-center justify-between z-10">
              <div>
                <h2 className="text-sm font-bold text-white">Order {selectedOrder.sourceOrderId}</h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  {selectedOrder.sourceLabel} · {new Date(selectedOrder.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </p>
              </div>
              <div className="flex items-center gap-2">
                {(selectedOrder.shopifyOrderId || selectedOrder.source === 'SHOPIFY' || selectedOrder.source === 'MERGED') && (
                  <a
                    href={`https://admin.shopify.com/store/tvbczb-ie/orders/${selectedOrder.shopifyOrderId ?? selectedOrder.sourceOrderId}`}
                    target="_blank"
                ...
                {selectedOrder.ccSourceOrderId && (
                  <a
                    href={`https://crm.checkoutchamp.com/customer/cs/orders/?orderId=${selectedOrder.ccSourceOrderId}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                <button onClick={() => setSelectedOrder(null)} className="p-1 text-gray-500 hover:text-gray-300">
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-900 rounded-lg p-3">
                  ...
                  <div className="text-lg font-bold text-white tabular-nums">{fmt$(selectedOrder.orderTotal)}</div>
                </div>
                <div className="bg-gray-900 rounded-lg p-3">
                  <div className="text-[10px] text-gray-500 uppercase font-medium">Shipping</div>
                  <div className="text-lg font-bold text-white tabular-nums">{fmt$(selectedOrder.totalShipping)}</div>
                </div>
              </div>
              ...
              <div className="bg-gray-900 rounded-lg p-4 space-y-2.5 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-300">{selectedOrder.customer?.fullName ?? selectedOrder.customer?.email}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Status</span>
                  <Badge label={selectedOrder.statusLabel} colorClass={selectedOrder.statusColor} />
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Type</span>
                  <span className="text-gray-300">{selectedOrder.ccOrderType ?? '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Campaign</span>
                  <span className="text-gray-300 truncate max-w-[180px]">{selectedOrder.campaignName ?? '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Pay Source</span>
                  <span className="text-gray-300">{selectedOrder.paySource ?? '—'}</span>
                </div>
                {selectedOrder.shopifyOrderId && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Shopify ID</span>
                    <span className="text-gray-400 font-mono">{selectedOrder.shopifyOrderId}</span>
                  </div>
                )}
                {selectedOrder.ccSourceOrderId && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">CC ID</span>
                    <span className="text-gray-400 font-mono">{selectedOrder.ccSourceOrderId}</span>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-900 rounded-lg overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-800">
                  <h3 className="text-xs font-semibold text-gray-400 uppercase">Line Items ({selectedOrder.items.length})</h3>
                </div>
